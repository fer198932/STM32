...
//	TIM_Cmd(TIM14,ENABLE);			//test
	...
	while(1)			// steps--
	{
		...
		{
			case 0:														// X轴
				xAxisPlusNum = plusNum;
				pwmNumTim3 = 0;
				xAxisStepFlag = 0;
				xAxisDIR_ON;												// 方向固定		
				xAxisClkSet(clkTemp>>(addSubCnt-1));		// 最低速度开始运行				
				...
				while(xAxisStepFlag != 2) ;					// 运行结束才退出
			break;			
			case 1:
				yAxisPlusNum = plusNum;
				pwmNumTim2 = 0;
				yAxisStepFlag = 0;
				yAxisDIR_ON;												// 方向固定		
				yAxisClkSet(clkTemp>>(addSubCnt-1));		// 最低速度开始运行			
				
				// 打开定时器开始运行
				TIM_Cmd(TIM2, ENABLE);							// 使能y
				TIM7->CNT = 0;											// 定时器重新开始计数
				TIM_Cmd(TIM7, ENABLE);							// 使能加减速定时器
				
				while(yAxisStepFlag != 2) ;					// 运行结束才退出
			break;
			case 2:
	...
	while(1)		// 关闭原来的测试程序
		{
//			key_Proc5axisComm((void*)0);   暂时关闭限位 byYJY
