...
//	while(xZeroFlag==1);		//¹éÁãÍê³É£»
	...
	TIM_Cmd(TIM3, DISABLE); 	//¹éÁãÍê³É£¬PWMÍ£Ö¹£
	TIM_Cmd(TIM14,DISABLE);		// ¹Ø±ÕÂö³å¼ÆÊı
	stepsFlag[0] = 0;
	enableXaxisCountINT();		//¿ªÆôxÖá¼ÆÊıÖĞ¶Ï£»
	delay_s(2);
}
...
void toolsFromORG2Start(void)	// ´ÓÁãµã·µ»ØÔ­À´¹¤¼şµÄÎ»ÖÃ
{
	...
	pwmNumTim3 = 0; 		
	xAxisClkSet(8000);			//xÖáÏÂÒÆµÄËÙ¶È£»
	...
	pwmNumTim2 = 0; 		
	yAxisClkSet(8000);			//yÖáÏÂÒÆµÄËÙ¶È£»
	...
	pwmNumTim4 = 0; 		
	zAxisClkSet(8000);			//zÖáÏÂÒÆµÄËÙ¶È£»
