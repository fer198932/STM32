...
extern u16 subPlusCnt[5];		// 减速阶段所需的脉冲数
...
extern u8 startSubFlag[5];			// 进入减速阶段的标记，0：不进入减速，1：开始减速
extern u16 correctSyncValue[5];		// 修正减速中间频率的减速阶段脉冲值
...
void EXTI0_IRQHandler(void)
{
	...
	else
	{
		...
		// 打开减速标记
		if((yAxisPlusNum - pwmNumTim2) == (subPlusCnt[1]+correctSyncValue[1]))		
		{
			TIM7->CNT = 0;
			startSubFlag[1] = 1;
		}
		...
		// 脉冲计数，用来记录在触发传感器前走了多少距离
		 if(stepsFlag[2])
		 ...
		 else 
			{
				...
				// 打开减速标记
				if((zAxisPlusNum - pwmNumTim4) == (subPlusCnt[2]+correctSyncValue[2]))		
				{
					TIM7->CNT = 0;
					startSubFlag[2] = 1;
				}
...
{ 
	...
	// 脉冲计数，用来记录在触发传感器前走了多少距离
	 ...
	 else 
	 {
		 ...
		 // 打开减速标记
			if((aAxisPlusNum - pwmNumTim9) == (subPlusCnt[3]+correctSyncValue[3]))		
			{
				TIM7->CNT = 0;
				startSubFlag[3] = 1;
			}
