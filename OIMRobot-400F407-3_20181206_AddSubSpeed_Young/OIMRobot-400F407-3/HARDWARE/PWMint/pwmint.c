...
//arr：自动重装值
//psc：时钟预分频数
void TIM9_PWM_Init(u32 arr,u16 psc)			//a轴
{		 					 
	//此部分需手动修改IO口设置
...
//arr：自动重装值
//psc：时钟预分频数
void TIM3_PWM_Init(u32 arr,u16 psc)		//X轴
{		 					 
	//此部分需手动修改IO口设置
	...
	TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM1; 				//选择定时器模式:TIM脉冲宽度调制模式2
 	TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable; 	//比较输出使能
	TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High; 		//输出极性:TIM输出比较极性低 byYJY
...
//arr：自动重装值
//psc：时钟预分频数
void TIM4_PWM_Init(u32 arr,u16 psc)			//z轴
{		 					 
	//此部分需手动修改IO口设置
...
//arr：自动重装值
//psc：时钟预分频数
void TIM2_PWM_Init(u32 arr,u16 psc)			//Y
{		 					 
	//此部分需手动修改IO口设置
...
{
	TIM3->PSC = psc;						// psc值决定频率
#ifdef DUTY_CYCLE
	TIM3->CCR1 = dRatio1;				// 占空比计算
	TIM3->CCR2 = dRatio2;				// 占空比计算
#endif
}
void yAxisSetPWM(u16 psc, u16 dRatio1, u16 dRatio2)
{
	TIM2->PSC = psc;						// psc值决定频率
#ifdef DUTY_CYCLE
	TIM2->CCR1 = dRatio1;				// 占空比计算
	TIM2->CCR4 = dRatio2;				// 占空比计算
#endif
}
void zAxisSetPWM(u16 psc, u16 dRatio1, u16 dRatio2)
{
	TIM4->PSC = psc;						// psc值决定频率
#ifdef DUTY_CYCLE
	TIM4->CCR3 = dRatio1;				// 占空比计算
	TIM4->CCR4 = dRatio2;				// 占空比计算
#endif
}
void aAxisSetPWM(u16 psc, u16 dRatio1, u16 dRatio2)
{
	TIM9->PSC = psc;						// psc值决定频率
#ifdef DUTY_CYCLE
	TIM9->CCR1 = dRatio1;				// 占空比计算
	TIM9->CCR2 = dRatio2;				// 占空比计算
#endif
}
void bAxisSetPWM(u16 psc, u16 dRatio1, u16 dRatio2)
{
	TIM12->PSC = psc;						// psc值决定频率
#ifdef DUTY_CYCLE
	TIM12->CCR1 = dRatio1;				// 占空比计算
	TIM12->CCR2 = dRatio2;				// 占空比计算
#endif
}
...
{
	// 基于频率调整的初始化方法
	TIM3_PWM_Init(ARR_VAL-1, calPSC(X_AXIS_MIN_CLK)-1);
	TIM2_PWM_Init(ARR_VAL-1, calPSC(Y_AXIS_MIN_CLK)-1);
	TIM4_PWM_Init(ARR_VAL-1, calPSC(Z_AXIS_MIN_CLK)-1);
	TIM9_PWM_Init(ARR_VAL-1, calPSC(A_AXIS_MIN_CLK)-1);
	TIM12_PWM_Init(ARR_VAL-1, calPSC(B_AXIS_MIN_CLK)-1);	
}
